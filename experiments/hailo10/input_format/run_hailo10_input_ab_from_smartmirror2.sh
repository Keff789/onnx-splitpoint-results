...
LOCAL_TEST="$SCRIPT_DIR/hailo10_input_format_ab.py"

H10_TARGET="${H10_TARGET:-nx@192.168.0.145}"
AB_WARMUP="${AB_WARMUP:-10}"
AB_FRAMES="${AB_FRAMES:-100}"
AB_TIMEOUT_MS="${AB_TIMEOUT_MS:-10000}"
AB_QUANT_REPETITIONS="${AB_QUANT_REPETITIONS:-50}"
REQUESTED_HEF="${1:-__AUTO__}"

STAMP="$(date -u +%Y%m%d_%H%M%S)"
REMOTE_TEST="/tmp/hailo10_input_format_ab_${STAMP}.py"
REMOTE_RESULT="/tmp/hailo10_input_format_ab_${STAMP}.json"
LOCAL_RESULT="$HOME/Downloads/hailo10_input_format_ab_${STAMP}.json"
...
for value_name in AB_WARMUP AB_FRAMES AB_TIMEOUT_MS AB_QUANT_REPETITIONS; do
  value="${!value_name}"
  [[ "$value" =~ ^[0-9]+$ ]] || {
...
echo "Policy: exactly one outstanding InferModel job"
echo "Copying isolated input-format A/B test..."
...
echo "Python: $PYTHON"
echo "HEF: $HEF"
echo "A/B: FLOAT32 input vs native HEF input; native output for both"
echo "Internal InferModel parallelism: disabled"
...
"$PYTHON" "$TEST_SCRIPT" \
  ...
  --quantization-repetitions "$QUANT_REPETITIONS"
REMOTE
REMOTE_RC=$?
set -e
