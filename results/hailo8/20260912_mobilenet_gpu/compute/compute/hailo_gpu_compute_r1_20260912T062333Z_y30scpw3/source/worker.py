...
def execute(output: Path, expected_venv: Path, family: str) -> int:
    ...
    report={'schema':'hailo-gpu-compute-smoke-r1','family':family,'status':'running',
    ...
    try:
        ...
        # Configure before creating any tensors or logical GPU devices.
        ...
        report['memory_growth']=tf.config.experimental.get_memory_growth(physical[0]);save()
        ...
        try:
            report['loaded_cuda_libraries']=sorted({line.split()[-1] for line in Path('/proc/self/maps').read_text().splitlines()
                if line.split() and any(t in line for t in ('libcuda.','libcudnn','libcublas','libcudart','libnvrtc'))})
